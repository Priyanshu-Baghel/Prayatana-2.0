    ...
    <main className="flex justify-center items-center min-h-screen p-4 bg-gray-50">
      <div className="w-full max-w-3xl bg-white rounded-xl shadow-lg overflow-hidden flex flex-col h-[80vh]">
        <div className="bg-primary p-4 text-white">
          <h1 className="text-xl font-semibold">AI Complaint Assistant</h1>
          <p className="text-sm opacity-80">Ask any Complaint's & Sahayata-Setu questions</p>
        </div>

        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.length === 0 && (
            <div className="text-center text-gray-500 my-8">
              <p className="text-lg font-medium">Welcome to the AI Complaint Assistant!</p>
              <p className="mt-2">Ask any questions about waste management, water supply and more.</p>
            </div>
          )}
          ...
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={message.role === 'user' ? 'user-bubble' : 'ai-bubble'}>
                {message.content || (message.role === 'assistant' && <span className="text-gray-400">Loading...</span>)}
              </div>
            </div>
          ))}
